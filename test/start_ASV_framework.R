...

{
  ...
  dup <- read.csv2(glue::glue("{wd}Framework/ArcticSDM/sibala_db/duplication/sibala_dup_species_name.csv"), 
...
# Process the results based on ASV level - 02.05.2024

# create the species list of this ASV, e.g., the first ASV
species_list <- na.omit(unlist(dup[2,-1]))
...
out <- 
  ...
  scale_fill_gradient(low = 'black', high = 'white', breaks = seq(0, 1, length = 5), na.value = "transparent", trans = "sqrt") +
  labs(fill = "Suitability") +
  labs(title=paste0("Mean MaxEnt ",time, " - ASV level"),
       x ="", y = "") +
  annotate("text", x=0, y=0, label= paste(matched_species_name, collapse = "\n"), size=4, hjust = 0, vjust = 0.4) +
  coord_sf() +
  theme_bw()
...
ggsave(file.path(output, paste0("Mean_MaxEnt ", time, " - ASV level - ", paste(target_taxa, collapse = ", "), ".png")),
       out, units = "cm", width = 30, height = 30)

write_stars(mean_values_rast, glue::glue("{output}/Mean_MaxEnt_{time}_ASV_level.tif"))
